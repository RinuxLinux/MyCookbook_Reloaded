...
PROFIL_DIR='/z/Dropbox/LABO-DBX/cv-lm/_profils'
DEBUG=0
...
import_tools() {
	...
	echo -en "${fg_cyan}${bg_black}Importer parsing tools ? [Y] ${normal}"
	read importer
...
select_profil() {
	# USAGE
	# import_profil "$newdir" "dev-alt-cs"
	newdir="$1"
	cie="$2"

	echo -e "Choisir un PROFIL:"
	echo -e "   1) dev alt CS"
	echo -e "   2) dev alt GENUINE"
	echo -e "   3) dev alt HIJACK"
	echo -e "   4) dev alt OP NUMERISATION"
	echo -e "   5) dev alt OP SAISIE"
	echo -e "   d) DEBUG"
	...
	cp -vu ${PROFIL_DIR}/${profil}/* $newdir
}
...
main() {
	...
	case $confirm in
		["yY"])
			...
			read -p "Créer le répertoire $newdir ?" rep369
			case $rep369 in
				["Yy"])
					setup_dir "$newdir"
					;;
				*)
					echo "Le repertoire ne sera pas créé."
					DEBUG=1
					;;
			esac

			read -p "Importer [P]ROFILS ou [C]USTOM ?" rep378
			case "$rep378" in 
				["Pc"])
					# here be profils selection
					if [ DEBUG -eq 1 ]; then
						echo -e "${fg_fat_red}Debug is ON: selection par PROFIL ${normal}"
						echo -e "${fg_fat_red}select_profil $newdir $cie ${normal}"
					else
				...
				["Cc"])
					if [ DEBUG -eq 1 ]; then
						echo -e "${fg_fat_red}Debug is ON: selection CUSTOM ${normal}"
					else
						# here be cv/lm selection
						select_cv "$newdir"
						import_template "$newdir"
